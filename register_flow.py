...
def fill_email_and_submit(timeout=45, log_callback=None, cancel_callback=None):
    ...
    # freemail 无 JWT，dev_token 为空串是正常的；仅要求 email 非空
    if not email:
...
def _page_stage_snapshot():
    """探测当前注册阶段：验证码页 / 资料页 / 其它。

    返回 dict: {url, hasProfile, hasOtp, hasEmail, buttons, inputs} 或 None。
    资料页判定放宽：密码框、given/family、中文名/姓、birthday 等任一可见即算。
    """
    try:
        return page.run_js(
            r"""
...
function textOf(n) {
  return (n.innerText || n.textContent || n.getAttribute('value') || n.getAttribute('aria-label') || n.getAttribute('placeholder') || '').replace(/\s+/g, ' ').trim();
}
const inputs = Array.from(document.querySelectorAll('input, textarea')).filter(isVisible);
const buttons = Array.from(document.querySelectorAll('button, [role="button"], input[type="submit"]'))
  .filter(isVisible).map(textOf).filter(Boolean).slice(0, 10);

const hasOtp = inputs.some((n) => {
  const maxLength = Number(n.maxLength || 0);
  const ac = String(n.autocomplete || '').toLowerCase();
  const name = String(n.name || n.getAttribute('data-testid') || '').toLowerCase();
  const type = String(n.type || '').toLowerCase();
  return (
    n.getAttribute('data-input-otp') === 'true' ||
    name === 'code' || name.includes('otp') || name.includes('verification') ||
    ac === 'one-time-code' ||
    maxLength === 1 ||
    (type === 'tel' && maxLength > 0 && maxLength <= 8)
  );
});

const hasPassword = inputs.some((n) => String(n.type || '').toLowerCase() === 'password' || String(n.autocomplete || '').toLowerCase().includes('password') || String(n.name || '').toLowerCase().includes('password'));
const hasGiven = inputs.some((n) => {
  const meta = [n.name, n.id, n.getAttribute('data-testid'), n.getAttribute('autocomplete'), n.getAttribute('aria-label'), n.getAttribute('placeholder')].join(' ').toLowerCase();
  return meta.includes('given') || meta.includes('first') || meta.includes('名') && !meta.includes('用户');
});
const hasFamily = inputs.some((n) => {
  const meta = [n.name, n.id, n.getAttribute('data-testid'), n.getAttribute('autocomplete'), n.getAttribute('aria-label'), n.getAttribute('placeholder')].join(' ').toLowerCase();
  return meta.includes('family') || meta.includes('last') || meta.includes('姓');
});
const hasBirth = inputs.some((n) => {
  const meta = [n.name, n.id, n.getAttribute('data-testid'), n.getAttribute('autocomplete'), n.getAttribute('aria-label'), n.getAttribute('placeholder'), n.type].join(' ').toLowerCase();
  return meta.includes('birth') || meta.includes('dob') || meta.includes('生日') || meta.includes('date');
});
const hasEmail = inputs.some((n) => {
  const meta = [n.name, n.type, n.getAttribute('data-testid'), n.getAttribute('autocomplete'), n.getAttribute('placeholder')].join(' ').toLowerCase();
  return meta.includes('email') || meta.includes('mail');
});
...
return false;
    ...
    def _already_on_profile(reason=""):
    ...
    if log_callback:
        log_callback(f"[*] 开始填写验证码: {code}")
    # 若拉取验证码期间用户/页面已自动跳转，直接成功
    if _already_on_profile("拉取后检测"):
        return code
    ...
    while time.time() < deadline:
        raise_if_cancelled(cancel_callback)
        # 每轮先看是否已在资料页（自动跳转 / 用户手动确认）
        if _already_on_profile("轮询检测"):
            return code
...
return 'not-ready';
        ...
        last_fill_state = str(filled or "")

        if filled == "not-ready":
            now = time.time()
            if log_callback and now - last_diag_at >= 4:
                last_diag_at = now
                snap = _page_stage_snapshot()
                if isinstance(snap, dict):
                    if snap.get("hasProfile"):
                        if log_callback:
                            log_callback(f"[*] 验证码页已消失，进入资料页: url={snap.get('url')}")
                        return code
                    log_callback(
                        f"[Debug] 验证码输入框未就绪: url={snap.get('url')} "
                        f"hasOtp={snap.get('hasOtp')} buttons={snap.get('buttons')}"
                    )
            sleep_with_cancel(0.5, cancel_callback)
            continue
...
const btn = buttons.find((node) => {
    const t = (node.innerText || node.textContent || node.getAttribute('value') || '').replace(/\s+/g, '').toLowerCase();
    return (
        t.includes('确认邮箱') ||
        t.includes('继续') ||
        t.includes('下一步') ||
    ...
    // 兜底：验证码页常见主按钮
    const t = (node.innerText || node.textContent || '').replace(/\s+/g, '').toLowerCase();
    return t.length > 0 && !t.includes('重新发送') && !t.includes('resend') && !t.includes('返回');
});

if (!btn) return 'no-button';
btn.focus();
btn.click();
return 'clicked:' + ((btn.innerText || btn.textContent || '').replace(/\s+/g, ' ').trim().slice(0, 40));
            """
        )

        if log_callback:
            log_callback(f"[*] 验证码提交动作: {clicked}")

        # 点击后轮询是否进入资料页（最长约 8s）
        if isinstance(clicked, str) and (
            clicked == "clicked"
            or clicked.startswith("clicked:")
            ...
            if advanced:
                ...
                return code
            if log_callback:
                log_callback(f"[Debug] 验证码提交后页面未前进，重试: {code} (last_fill={last_fill_state})")
            sleep_with_cancel(0.8, cancel_callback)
            continue
...
const givenInput = pickInput('input[data-testid="givenName"], input[name="givenName"], input[autocomplete="given-name"], input[name*="given" i], input[aria-label*="名"], input[placeholder*="名"]')
  || Array.from(document.querySelectorAll('input')).find((n) => {
      if (!isVisible(n) || n.disabled || n.readOnly) return false;
      const meta = [n.name, n.id, n.getAttribute('data-testid'), n.getAttribute('autocomplete'), n.getAttribute('aria-label'), n.getAttribute('placeholder')].join(' ').toLowerCase();
      return (meta.includes('given') || meta.includes('first') || (meta.includes('名') && !meta.includes('用户'))) && String(n.type||'').toLowerCase() !== 'password';
    }) || null;
const familyInput = pickInput('input[data-testid="familyName"], input[name="familyName"], input[autocomplete="family-name"], input[name*="family" i], input[aria-label*="姓"], input[placeholder*="姓"]')
  || Array.from(document.querySelectorAll('input')).find((n) => {
      if (!isVisible(n) || n.disabled || n.readOnly) return false;
      const meta = [n.name, n.id, n.getAttribute('data-testid'), n.getAttribute('autocomplete'), n.getAttribute('aria-label'), n.getAttribute('placeholder')].join(' ').toLowerCase();
      return (meta.includes('family') || meta.includes('last') || meta.includes('姓')) && String(n.type||'').toLowerCase() !== 'password';
    }) || null;
const passwordInput = pickInput('input[data-testid="password"], input[name="password"], input[type="password"], input[autocomplete="new-password"]');

if (!passwordInput) return 'not-ready';
// 名/姓可能文案不同；至少密码必须有。名姓都找不到时也继续，后面会诊断
if (!givenInput && !familyInput) return 'not-ready-name';

const ok1 = givenInput ? setInputValue(givenInput, givenName) : true;
const ok2 = familyInput ? setInputValue(familyInput, familyName) : true;
...
const submitBtn = buttons.find((node) => {
    const t = (node.innerText || node.textContent || '').replace(/\s+/g, '').toLowerCase();
    return t.includes('完成注册') || t.includes('创建账户') || t.includes('注册') || t.includes('继续') || t.includes('下一步') || t.includes('signup') || t.includes('createaccount') || t.includes('continue') || t.includes('next') || t.includes('submit');
});
...
return String(cfInput.value || '').trim().length;
            ...
            if filled in ("ready-to-submit", "filled-no-submit"):
                ...
                not_ready_streak = 0
                if log_callback:
                    log_callback(f"[*] 资料表单已写入: {given_name} {family_name}")
            elif filled == "fill-failed" and log_callback:
                log_callback("[Debug] 资料输入失败，重试中...")
                sleep_with_cancel(0.5, cancel_callback)
                continue
            elif filled in ("not-ready", "not-ready-name"):
                not_ready_streak += 1
                now = time.time()
                if log_callback and (now - last_diag_at >= 4 or not_ready_streak == 1):
                    last_diag_at = now
                    snap = _page_stage_snapshot()
                    if isinstance(snap, dict):
                        log_callback(
                            "[Debug] 资料页未就绪: "
                            f"state={filled}; url={snap.get('url')}; "
                            f"hasOtp={snap.get('hasOtp')}; hasPassword={snap.get('hasPassword')}; "
                            f"hasGiven={snap.get('hasGiven')}; hasFamily={snap.get('hasFamily')}; "
                            f"buttons={snap.get('buttons')}"
                        )
                        if snap.get("hasOtp"):
                            raise Exception(
                                "仍停在验证码页，未能进入资料填写（验证码可能未真正通过）"
                            )
                sleep_with_cancel(0.5, cancel_callback)
                continue
            else:
                # 未知状态也打日志，避免静默空转
                if log_callback and time.time() - last_diag_at >= 4:
                    last_diag_at = time.time()
                    log_callback(f"[Debug] 资料填写状态: {filled}")
                sleep_with_cancel(0.5, cancel_callback)
                continue

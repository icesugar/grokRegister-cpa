...
def check_email_api(provider: str, config: dict, http_get: Callable, http_post: Callable) -> CheckResult:
    ...
    try:
        if provider == "cloudflare":
            ...
            backend = str(config.get("cloudflare_backend", "temp_email") or "temp_email").strip().lower()
            api_key = str(config.get("cloudflare_api_key", "") or "")
            # freemail：X-Admin-Token + /api/domains
            if backend in ("freemail", "free-mail", "free_mail"):
                if not api_key:
                    return "邮箱API", False, "freemail 未配置 cloudflare_api_key（X-Admin-Token）"
                try:
                    from email_providers import freemail as freemail_provider

                    # 连通性检查时用 config.proxy（若有）；注册时另走代理池线程本地
                    ...
                    client = freemail_provider.FreemailClient(
                        base, api_key, get_proxies=_proxies
                    )
                    domains = client.get_domains()
                    return "邮箱API", True, f"freemail 可达 domains={len(domains)}"
                except Exception as exc:
                    return "邮箱API", False, f"freemail: {exc}"
            # 试 domains 或根
